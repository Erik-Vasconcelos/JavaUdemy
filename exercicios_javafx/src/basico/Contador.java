...
public class Contador extends Application {
	...
	private void atualizarLabel(Label label) {
		label.setText(Long.toString(contador));
	...
	public void start(Stage primaryStage) throws Exception {
		...
		botaoDecremento.setOnAction(e -> {decremento();
			atualizarLabel(labelNumero);
		});
		
		Button botaoIncremento = new Button("+");
		botaoIncremento.getStyleClass().add("botao");
		botaoIncremento.setOnAction(e -> {incremento();
			atualizarLabel(labelNumero);
		});
		...
		String font = "https://fonts.googleapis.com/css2?family=Oswald";
		Scene cenaPrincipal = new Scene(boxConteudo, 400, 400);
		cenaPrincipal.getStylesheets().add(caminhoDoCss);
		cenaPrincipal.getStylesheets().add(font);
